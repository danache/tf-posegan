...
class train_class():
    def __init__(self, model, nstack=4, batch_size=32,learn_rate=2.5e-4, decay=0.96, decay_step=2000,gamma=0.5,
            ...
            for i in range(len(self.gpu)):
                ...
                with tf.device(("/gpu:%d" % self.gpu[i])):
                    with tf.name_scope('gpu_%d' % ( self.gpu[i])) as scope:
                        ...
                        ####### groundTruth output #######
                        self.discriminator_groundTruth_output.append(self.model.discrim(inputs=self.concat_trainmini_heatmap[i],
                                                                                             reuse=dis_flag))
                        ...
                        self.generator_output.append(self.model.build(self.train_img_lst[i],reuse=hg_flag))
                        hg_flag = True
